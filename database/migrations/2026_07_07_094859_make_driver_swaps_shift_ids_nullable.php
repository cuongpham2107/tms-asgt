...
return new class extends Migration
{
    ...
    {
        // Detect if already migrated (idempotent)
        $cols = DB::select("PRAGMA table_info('driver_swaps')");
        $hasNullableFromShift = collect($cols)->contains(
            fn ($c) => $c->name === 'from_shift_id' && $c->notnull === 0,
        );

        if (! $hasNullableFromShift) {
            ...
            DB::statement('CREATE INDEX IF NOT EXISTS driver_swaps_trip_id_index ON driver_swaps (trip_id)');
            DB::statement('CREATE INDEX IF NOT EXISTS driver_swaps_from_driver_id_index ON driver_swaps (from_driver_id)');
            DB::statement('CREATE INDEX IF NOT EXISTS driver_swaps_to_driver_id_index ON driver_swaps (to_driver_id)');
        }

        // Add start_location_id and end_location_id to trips (idempotent)
        $tripCols = DB::select("PRAGMA table_info('trips')");
        $tripColNames = collect($tripCols)->pluck('name');

        if (! $tripColNames->contains('start_location_id')) {
            DB::statement('ALTER TABLE trips ADD COLUMN start_location_id INTEGER REFERENCES locations(id) ON DELETE SET NULL');
        }

        if (! $tripColNames->contains('end_location_id')) {
    ...
    {
        // Remove start/end location columns from trips
        ...
        // Revert driver_swaps: from_shift_id NOT NULL
